...
pub fn tuple_to_point(tpl: &TupleVec) -> Vec3 {
    Vec3::new(tpl.0, tpl.1, tpl.2)
}

/// Convert a `TupleVec` to a `nalgebra` vector.
pub fn tuple_to_vector(tpl: &TupleVec) -> Vec3 {
    Vec3::new(tpl.0, tpl.1, tpl.2)
}
...
pub fn generate_aligned_boxes() -> Vec<UnitBox> {
    ...
    for x in -10..11 {
        shapes.push(UnitBox::new(x, Vec3::new(x as f32, 0.0, 0.0)));
    }
    shapes
}
...
pub fn traverse_some_bh<BH: BoundingHierarchy>() {
...

    {
        ...
        let origin = Vec3::new(6.0, 0.5, 0.0);
...
/* impl FromRawVertex for Triangle {
    fn process(
        ...
        // Convert the vertices to `Vec3`s.
        let points = vertices
            .into_iter()
